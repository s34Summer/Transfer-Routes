...
function refreshUploadBanners() {
    const doneArrivals = extractedManifests.some(m => m.type === 'arrival');
    const doneDepartures = extractedManifests.some(m => m.type === 'departure');
    const doneDrivers = Object.keys(excelDriversMap).length > 0;
    const doneAirport = Object.keys(airportStaffMap).length > 0;

    const states = {
        uploadBannerArrivals: doneArrivals,
        uploadBannerDepartures: doneDepartures,
        uploadBannerDrivers: doneDrivers,
        uploadBannerAirport: doneAirport
    };

    Object.keys(states).forEach(id => {
        const el = document.getElementById(id);
        if (el) el.classList.toggle('hidden', states[id]);
    });
}

function dismissUploadBanner(id) {
    const el = document.getElementById(id);
    ...
    for (let i = 0; i < processedLines.length; i++) {
        ...
        if (refMatchOnLine) {
            ...
            if (!targetStop.passengers.some(p => p.refNum === refNum)) {
                ...
                targetManifest.searchString += ` ${globalVoucherNo} ${leadName} ${refNum} ${currentHotelName} ${passengerOperator} ${manifestDate}`.toLowerCase();
            }
        }
    }
}
    ...
    extractedManifests.forEach((manifest, index) => {
        ...
        manifest.stops.forEach((stop, stopIndex) => {
            ...
            stop.passengers.forEach(p => {
                ...
                const matchesVoucher = manifest.voucherId.toLowerCase().includes(lowerQuery);

                if (lowerQuery && !matchesName && !matchesRef && !matchesHotel && !matchesOp && !matchesStaff && !matchesVoucher) return;
        ...
        const highlightedVoucher = highlightText(displayVoucher, query);
            ...
            <div class="voucher-head" onclick="toggleVoucherCollapse('${contentBlockId}', '${arrowIconId}')">
                <div class="voucher-head-left">
                    ...
                    <div class="vou-meta">
                        ...
                        <span onclick="event.stopPropagation(); showFullVoucherModal(${index})" title="Tap to view the full voucher" class="vou-badge">${highlightedVoucher}</span>
                    </div>
